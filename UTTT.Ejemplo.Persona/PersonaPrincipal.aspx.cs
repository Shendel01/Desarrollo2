...
namespace UTTT.Ejemplo.Persona
{
    public partial class PersonaPrincipal : System.Web.UI.Page
    {
        ...
        {
            AppDomain.CurrentDomain.FirstChanceException += (hamburguesas, ex) =>
            {
                StringBuilder error_hamburguesa = new StringBuilder();
                error_hamburguesa.AppendLine(
                    ex.Exception.GetType().FullName);
                error_hamburguesa.AppendLine(
                    ex.Exception.Message);

                System.Diagnostics.StackTrace guardar_stacktrace = new System.Diagnostics.StackTrace();

                error_hamburguesa.AppendLine(
                    guardar_stacktrace.ToString());

                error_hamburguesa.AppendLine();

                error = error_hamburguesa.ToString();
